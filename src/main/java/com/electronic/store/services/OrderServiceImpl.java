...
import com.electronic.store.dto.*;
...
public class OrderServiceImpl implements OrderService{
    ...
    public OrderDto getOrder(String orderId) {
        Order order = this.orderRepository.findById(orderId).orElseThrow(() -> new RuntimeException("Order not found !!"));
    ...
    public OrderDto createOrder(CreateOrderRequest orderDto) {
        ...
        //fetch cart
        Cart cart = cartRepository.findById(cartId).orElseThrow(() -> new RuntimeException("Cart with given id not found on server !!"));

        List<CartItem> cartItems = cart.getItems();

        if (cartItems.size() <= 0) {
            throw new RuntimeException("Invalid number of items in cart !!");
...
//            CartItem->OrderItem
    ...
    public OrderDto updateOrder(String orderId, OrderUpdateRequest request) {
        ...
        //get the order
        Order order = orderRepository.findById(orderId).orElseThrow(() -> new RuntimeException("Invalid update data"));
